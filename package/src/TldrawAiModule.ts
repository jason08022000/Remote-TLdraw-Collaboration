...
import {
	TLAiChange, TLAiContent, TLAiMessages, TLAiPrompt, TLAiCreateDecisionMatrixChange,
...
import { createDecisionMatrix } from './handlers/createDecisionMatrix'
...
export class TldrawAiModule {
	...
	async generate(prompt: string | { message: TLAiMessages; stream?: boolean }) {
		...
		const handleChanges = (changes: TLAiChange[]) => {
			...
			// 逐个应用
			for (const c of changes) handleChange(c)
		}
		...
		try {
			switch (change.type) {
				...
				case 'createLinearDiagram': {
					createLinearDiagram(editor, change as TLAiCreateLinearDiagramChange)
					break
				}
				case 'createDecisionMatrix': {
					createDecisionMatrix(editor, change as TLAiCreateDecisionMatrixChange)
					break
				}
				default:
